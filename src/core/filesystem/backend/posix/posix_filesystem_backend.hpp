...
// POSIX 文件系统后端实现
class PosixFilesystemBackend : public FilesystemBackend
{
public:
    // 打开文件，如果打开失败则返回错误
    [[nodiscard]]
    std::expected<FileHandle, FilesystemError> open(const std::filesystem::path & path, const FileOpenOptions & options) override;
